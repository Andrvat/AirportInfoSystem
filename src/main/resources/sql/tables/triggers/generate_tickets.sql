...
DECLARE
    ...
    CURSOR cur_capacity is
        SELECT airplane_id, people_capacity FROM (SELECT * FROM AIRPLANE JOIN AIRPLANE_TYPE USING (airplane_type_id));
BEGIN
    IF :NEW.AIRPLANE_ID <> :OLD.AIRPLANE_ID OR :OLD.AIRPLANE_ID IS NULL THEN
        trig_airplane_id := :NEW.AIRPLANE_ID;
        OPEN cur_capacity;
        LOOP
            FETCH cur_capacity INTO loop_airplane_id, loop_capacity;
        ...
        FOR I IN 1..trig_capacity
            LOOP
                INSERT INTO TICKET (DEPARTURE_ID, SEAT, TICKET_STATUS_ID, BAG_MAX_CAPACITY) VALUES (:NEW.DEPARTURE_ID, I, 1, NULL);
            END LOOP;
    END IF;
END;